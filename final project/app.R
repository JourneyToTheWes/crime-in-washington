...
year.length <- range(crime.pop.front$year)
year.width <- range(crime.pop.front$year)

ui <- fluidPage(
    titlePanel("WA State Criminal Records"),
    sidebarLayout(
      sidebarPanel(
        selectInput('County', label = 'county', choices =c("KING"), selected = "ADAMS"),
        sliderInput('year.width', label = 'Year', min = year.width[1], max = year.width[2], value = year.width)
      ),
    mainPanel(
        textOutput('mytable'),
        p('This graph displays the crimes committed within certain counties of Washington State'),
...
server <- function(input, output) {
  filtered <- reactive({
    data <- crime.pop.front %>% 
      filter(year == input$year) %>% 
      filter(year.width > input$year.width[1] & year.width < input$year.width[2])
      return(data)
  })
  ...
  output$plot <- renderPlot({
    ...
    output$table <- renderDataTable({
      output$mytable <- renderText(input$mytable)
    })
  })
